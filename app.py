# ─────────────────────────────────────────────────────────────
# app.py  —  F1 Race Predictor Flask Backend
# Run with:  python app.py
# Then open f1_predictor_v2.html in your browser
# ─────────────────────────────────────────────────────────────
...
# Load feature column order from Phase 2
FEATURE_COLS = [
...
# Team average finish positions (from your training data)
TEAM_AVG = {
...
def predict():
    try:
        ...
        # Encode driver and team (handle unseen labels gracefully)
        try:
            driver_enc = int(driver_encoder.transform([driver])[0])
        except ValueError:
            driver_enc = 0
        ...
        # Build feature row — same order as Phase 2
        quali_time   = 88.0 + quali_gap        # approximate best quali time
        driver_form  = DRIVER_FORM.get(driver, 10.0)
        team_avg     = TEAM_AVG.get(team, 10.0)
        wet_x_grid   = is_wet * grid_pos
        season_prog  = 0.5                      # mid-season default
        driver_count = 50                       # experienced driver default

        row = pd.DataFrame([{
            "GridPos":         grid_pos,
            "QualiPos":        quali_pos,
            "BestQualiTime":   quali_time,
            "QualiGapToPole":  quali_gap,
            "DriverRaceCount": driver_count,
            "DriverRecentForm":driver_form,
...
def home():
    return jsonify({
        "status":  "F1 Predictor API is running!",
        "endpoints": ["/predict", "/predict-grid"]
    })


if __name__ == "__main__":
    print("\n" + "="*45)
    print("  F1 Race Predictor API")
    print("  Running at http://localhost:5000")
    print("  Open f1_predictor_v2.html in browser")
    print("="*45 + "\n")
